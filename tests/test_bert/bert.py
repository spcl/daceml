...
print("Close: {}, {}".format(np.allclose(outputs[1], unstack_0),
                             np.allclose(outputs[0], unstack_1)))
...
for i in range(darray.shape[0]):
    for j in range(darray.shape[1]):
        if darray[i][j] == False:
            error_count += 1
            print(i, j, ': onnxrt = ', unstack_0[i][j], 'dace = ',
                  outputs[1][i][j])
...
for i in range(darray.shape[0]):
    for j in range(darray.shape[1]):
        if darray[i][j] == False:
            error_count += 1
            print(i, j, ': onnxrt = ', unstack_1[i][j], 'dace = ',
                  outputs[0][i][j])
