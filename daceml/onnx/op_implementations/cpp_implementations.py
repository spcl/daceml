...
class DetectLN(transformation.Transformation):
    ...
    def expressions():
        graph_nodes = [
            DetectLN.input, DetectLN.reduce_mean, DetectLN.onnx_3,
            DetectLN.sub, DetectLN.onnx_4, DetectLN.pow, DetectLN.onnx_6,
            DetectLN.reduce_mean_2, DetectLN.onnx_7, DetectLN.add,
            DetectLN.onnx_9, DetectLN.sqrt, DetectLN.onnx_10, DetectLN.div,
            DetectLN.onnx_11, DetectLN.mul, DetectLN.onnx_12, DetectLN.add_2,
            DetectLN.output
        ]
    ...
    def apply(self, sdfg: dace.SDFG):
        ...
        state.add_edge(input, None, ln_node, "_X",
                       sdfg.make_array_memlet(input.data))
        state.add_edge(weight, None, ln_node, "_scale",
                       sdfg.make_array_memlet(weight.data))
        state.add_edge(bias, None, ln_node, "_bias",
                       sdfg.make_array_memlet(bias.data))

        state.add_edge(ln_node, "_Y", output, None,
                       sdfg.make_array_memlet(output.data))
        mean_name, _ = sdfg.add_array("detected_layernorm_mean",
                                      [*input_shape[:axis], 1],
                                      dace.float32,
                                      storage=dtypes.StorageType.GPU_Global,
                                      transient=True,
                                      find_new_name=True)
        std_name, _ = sdfg.add_array("detected_layernorm_std",
                                     [*input_shape[:axis], 1],
                                     dace.float32,
                                     storage=dtypes.StorageType.GPU_Global,
                                     transient=True,
                                     find_new_name=True)
        state.add_edge(ln_node, "_mean", state.add_write(mean_name), None,
                       sdfg.make_array_memlet(mean_name))
        state.add_edge(ln_node, "_inv_std_var", state.add_write(std_name),
                       None, sdfg.make_array_memlet(std_name))
...
def add_ln_tasklet_bwd(state: dace.SDFGState, input_shape, axis):
    ...
    tasklet = state.add_tasklet(
        "layernorm", {i: dace.pointer(dace.float32)
