...
_placeholder_name_to_onnx_op: Dict[str, nodes.Node] = {}
...
def get_replaced_onnx_op(name: str) -> nodes.Node:
    ...
    return _placeholder_name_to_onnx_op[name]
...
def register_replacement(replaced_name: str, inputs: List[type] = None, outputs: List[type] = None):
    placeholder_name = replaced_name + 'Placeholder'

    _modules_to_replace[replaced_name] = placeholder_name

    schema_dict = make_schema_dict(
        replaced_name, placeholder_name, inputs, outputs)
