...
class ONNXOp(nd.LibraryNode):
    ...
    def register_implementations(self):
        """Register all implementations for this node"""

        # Define own register_implementation because the one in dace attempts to instantiate the node
        # which fails in the case of ONNX ops
        def _register_implementation(self, name, transformation_type):
            self.implementations[name] = transformation_type
            ...
            transformation_type._match_node = self
        ...
        _register_implementation(self, 'onnxruntime', Expansion)
        self.implementation = 'onnxruntime'
