...
import dace
import torch
from dace import data as dt, dtypes, nodes
...
from daceml.onnx import ONNXModel, parse_variadic_param
...
class HorizontalEinsumFusion(transformation.Transformation):
    ...
    @staticmethod
    def expressions():
        return [sdutil.node_path_graph(HorizontalEinsumFusion.top, HorizontalEinsumFusion.access, HorizontalEinsumFusion.bot)]

    def can_be_applied(self, graph: dace.sdfg.graph.OrderedMultiDiConnectorGraph,
    ...
    def apply(self, sdfg: dace.SDFG):
        ...
        if len(top_inputs) == 1:
            ...
            new_input_str = "".join(top_idx_to_bottom_idx[c] for c in top_inputs[0])
            bot_inputs[input_idx] = new_input_str
            bot.equation = f'{",".join(bot_inputs)}->{bot_output}'

            top_input_edge = state.in_edges(top)[0]
            state.add_edge(top_input_edge.src, top_input_edge.src_conn, bot, connector_on_bot, top_input_edge.data)
            ...
            new_output_str = "".join(bottom_idx_to_top_idx[c] for c in bot_output)
            top.equation = f'{",".join(top_inputs)}->{new_output_str}'

            bot_output_edge = state.out_edges(bot)[0]
            state.add_edge(top, "Output", bot_output_edge.dst, bot_output_edge.dst_conn, bot_output_edge.data)
            state.remove_node(access)
